...
logisticmodel <- glm(IsEdge ~ AdamicAdar+WeightedKatz,family=binomial(link='logit'),data=FoldedDataTrain)
svmmodel <- svm(IsEdge ~ AdamicAdar+WeightedKatz, data = FoldedDataTrain)
...
fitted.results <- predict(svmmodel,newdata=FoldedDataTrain)
fitted.results <- ifelse(fitted.results > 0.5,1,0)

trainingMisClassificationError <- mean(fitted.results != FoldedDataTrain$IsEdge)
print(paste('SVM Training Error:',trainingMisClassificationError))
...
for(i in 1:numFolds){
  ...
  crosslogisticmodel <- glm(IsEdge ~ AdamicAdar+WeightedKatz,family=binomial(link='logit'),data=trainData)
  ...
  crosssvmmodel <- svm(IsEdge ~ AdamicAdar+WeightedKatz, data = FoldedDataTrain)
  fitted.results <- predict(crosssvmmodel,newdata=testData,type='response')
  fitted.results <- ifelse(fitted.results > 0.5,1,0)
  misClassificationError <- mean(fitted.results != testData$IsEdge)
  svmvalidationErrors[i] <- misClassificationError
}
validationError <- mean(validationErrors)
svmvalidationError <- mean(svmvalidationErrors)
print(paste('Validation Error:',validationError))
print(paste('svm Validation Error:',svmvalidationError))
